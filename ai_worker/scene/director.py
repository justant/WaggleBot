...
intro / body(image_text|text_only|image_only) / outro 씬을 순서대로 배분한다.

씬 흐름:
    intro(hook) → [image_text | text_only | image_only] × N → outro(closer + mood 아웃트로 이미지)
...
- "i2v": 이미지 기반 비디오 생성 (intro image_text에서 I2V 평가 통과 시)
...
SceneType = Literal["intro", "image_text", "text_only", "image_only", "outro"]
