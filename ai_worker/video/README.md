...
ComfyUI 서버에 LTX-2 워크플로우를 제출하고, 결과 mp4 클립을 수신하여
...
│   ├── i2v_ltx2.json              # I2V 풀 모델
│   ├── i2v_ltx2_distilled.json    # I2V Distilled 모델 (4차 폴백)
...
**LLM 호출**: `call_ollama_raw(prompt=..., max_tokens=80, temperature=0.2)`
...
- **Phase 7 LLM 호출 제로 보장**: 원본과 함께 `simplified` 버전도 미리 생성 → `scene.video_prompt_simplified`
...
Phase 7 중간 상태를 저장하여 중단 시 재개할 수 있다.

```python
@dataclass
class VideoCheckpoint:
    phase: int = 7
    video_scenes_done: list[int]       # 완료된 씬 인덱스 목록
    video_clips: dict[str, str]        # {"0": "path.mp4", ...} (씬 인덱스→클립 경로)
    total_scenes: int = 0

    def to_dict(self) -> dict: ...     # 직렬화
    @classmethod
    def from_dict(cls, data) -> "VideoCheckpoint": ...  # 역직렬화
```

체크포인트가 있으면 `generate_all_clips()`에서 완료된 씬의 클립 파일 존재를 확인하고 스킵한다.
파일이 유실된 씬은 재생성한다.
...
4. **콜백**: `on_scene_complete(scene_idx, clip_path)` — 씬 완료 시 체크포인트 저장용
5. **실패 씬 병합**: 모든 씬 처리 후 실패 씬의 대본을 인접 씬에 병합
...
| `interrupt_current()` | POST `/interrupt` — 실행 중인 워크플로우 중단 | - |
| `clear_queue()` | POST `/queue` `{clear: true}` — 대기 큐 삭제 | - |
...
| `generate_i2v()` | Image-to-Video | `i2v_ltx2.json` 또는 `i2v_ltx2_distilled.json` |
...
2. _patch_workflow(workflow, params)
   └── 노드 inputs의 키/값을 params로 교체
       ...
       - 값 매칭 (화이트리스트): inputs의 값이 _PLACEHOLDER_WHITELIST에 포함되고
...
4. GET /history/{prompt_id} polling (3초 간격)
   ├── completed=True → 완료
   ├── status_str="error" → RuntimeError
   └── 10회 연속 연결 실패 → RuntimeError (크래시 감지)
...
6. _wait_for_file(path)
   └── Docker 볼륨 동기화 대기 (크기 안정 2회 확인, 30초 타임아웃)
```

### _PLACEHOLDER_WHITELIST
...
이 목록에 없는 문자열은 placeholder로 취급하지 않는다 (의도치 않은 값 교체 방지).

### I2V 이미지 처리

1. `_resize_image()`: 종횡비 3:1 초과 시 center-crop → target보다 크면 LANCZOS 리사이즈
2. `_upload_image()`: POST `/upload/image` (multipart/form-data)
3. 워크플로우에 `init_image` 파라미터로 서버 내 파일명 전달
4. finally 블록에서 리사이즈 임시 파일 삭제
...
| 종횡비 | 0.20 | max/min <= 2.0 | 2.0~3.0 → 0.10, >3.0 → 0.00 (강제) |
| 텍스트 밀도 | 0.30 | 텍스트 캡처본 아님 | bimodal 히스토그램 > 70% + 엣지 <3% → 0.00 |
| 색상 다양성 | 0.15 | RGB std > 30 | 15~30 → 0.07, <15 → 0.00 |
| 엣지 밀도 | 0.15 | 엣지 픽셀 > 5% | 2~5% → 0.07, <2% → 0.00 |

**극단적 종횡비 가드**: 3:1 초과 시 다른 항목 점수와 무관하게 `score=0.0` 강제 반환.
(CUBLAS 에러 유발 방지)
...
`@functools.cache` 데코레이터로 캐시된 런타임 프로빙:
1. `h264_nvenc`로 테스트 인코딩 시도 (nullsrc, 0.1초)
2. 성공 → True, 실패 → False (libx264 폴백)

### _resolve_intermediate_codec() → tuple[str, list[str]]

GPU 코덱 자동 감지:
- NVENC 가능 → `("h264_nvenc", ["-c:v", "h264_nvenc", "-preset", "p1", "-pix_fmt", "yuv420p"])`
- NVENC 불가 → `("libx264", ["-c:v", "libx264", "-preset", "ultrafast", "-pix_fmt", "yuv420p"])`
...
### Full 모드 (VIDEO_WORKFLOW_MODE="full", 기본)

| 단계 | 해상도 | 프레임 | 스텝 | CFG | 모델 | 프롬프트 |
|------|--------|--------|------|-----|------|---------|
| 1차 | 1280x720 | 97 (~4초) | 20 | 3.5 | Full | 원본 |
| 2차 | 1280x720 | 97 (~4초) | 20 | 3.5 | Full | 단순화 |
| 3차 | 768x512 | 65 (~2.7초) | 15 | 3.5 | Full | 단순화 |
| 4차 | 768x512 | 65 (~2.7초) | 8 | 1.0 | Distilled | 단순화 |

### Distilled 모드 (VIDEO_WORKFLOW_MODE="distilled")

| 단계 | 해상도 | 프레임 | 스텝 | CFG | 모델 | 프롬프트 |
|------|--------|--------|------|-----|------|---------|
| 1차 | 1280x720 | 97 | 8 | 1.0 | Distilled | 원본 |
| 2차 | 1280x720 | 97 | 8 | 1.0 | Distilled | 단순화 |
| 3차 | 768x512 | 65 | 8 | 1.0 | Distilled | 단순화 |
| 4차 | 768x512 | 65 | 8 | 1.0 | Distilled | 단순화 |

### CUBLAS 에러 처리

- `"cublas"` 키워드 감지 시 `_cleanup_after_cublas_error()` 실행
- 정리 순서: `interrupt_current()` → `clear_queue()` → POST `/free` (모델 언로드+메모리 해제) → 5초 대기
- **2회 연속** CUBLAS 에러 시 CUDA 컨텍스트 오염 판단 → 씬 조기 스킵

### OOM 복구

`"out of memory"` 에러 감지 시 `torch.cuda.empty_cache()` + `gc.collect()` 즉시 실행.
...
5. text_lines 항목이 dict가 아니면 `{"text": str(line), "audio": None}`으로 변환
...
├── t2v_ltx2_distilled.json    # T2V Distilled 모델 (4차 폴백)
├── i2v_ltx2.json              # I2V 풀 모델
├── i2v_ltx2_distilled.json    # I2V Distilled 모델 (I2V 4차 폴백)
└── t2v_ltx2_upscale.json      # 2-Stage 업스케일 (테스트용)
```
...
| 파라미터 키 | 대상 노드 | 설명 |
|------------|----------|------|
| `positive_prompt` | CLIPTextEncode | 비디오 프롬프트 (값 매칭, 화이트리스트) |
| `negative_prompt` | CLIPTextEncode | 네거티브 프롬프트 (값 매칭, 화이트리스트) |
| `init_image` | LoadImage (I2V만) | 업로드된 이미지 파일명 (값 매칭, 화이트리스트) |
| `width`, `height` | EmptyLTXVLatentVideo | 출력 해상도 (키 매칭) |
| `length`, `frames_number` | EmptyLTXVLatentVideo | 프레임 수 (키 매칭) |
| `frame_rate` | LTXVConditioning, VHS_VideoCombine | FPS (키 매칭) |
| `steps` | ManualSigmas | 샘플링 스텝 수 (키 매칭) |
| `cfg` | SamplerCustomAdvanced | CFG 스케일 (키 매칭) |
| `noise_seed` | SamplerCustomAdvanced | 랜덤 시드 (키 매칭) |
| `strength` | LTXVImgToVideoInplace (I2V만) | 변형 강도 (키 매칭) |
...
| `VIDEO_WORKFLOW_MODE` | `full` | 워크플로우 모드 (`full` / `distilled`) |
...
7. FFmpeg 코덱: NVENC 런타임 감지 → 불가 시 libx264 폴백
...
| 클래스 | 필드 | 용도 |
|--------|------|------|
| `VideoCheckpoint` | phase, video_scenes_done, video_clips, total_scenes | Phase 7 체크포인트 (중단 재개) |
| `VideoGenerationResult` | scene_index, success, clip_path, attempts, failure_reason, merged_into | 씬별 생성 결과 |

#### VideoManager 클래스

| 메서드 | 접근 | 역할 |
|--------|------|------|
| `__init__(comfy_client, prompt_engine, config)` | public | 클라이언트/엔진/설정 주입 |
| `generate_all_clips(scenes, mood, post_id, ...)` | public | 전체 씬 순차 생성 + 실패 병합 |
| `_generate_single_clip(scene, scene_index, post_id)` | private | 단일 씬 4단계 재시도 |
| `_resolve_attempt_params(scene, attempt, post_id, scene_index)` | private | 시도별 파라미터 결정 (해상도/프롬프트/모델) |
| `_merge_failed_scenes(scenes, results)` | private | 실패 씬 삭제 + text_lines 병합 |
| `_cleanup_after_cublas_error()` | private | CUBLAS 에러 후 ComfyUI 상태 복구 |
| `_is_distilled_mode()` | private | VIDEO_WORKFLOW_MODE 확인 |

### comfy_client.py (~491행)
...
| 메서드 | 접근 | 역할 |
|--------|------|------|
| `__init__(base_url, output_dir)` | public | 서버 URL, client_id(UUID), 워크플로우/출력 디렉터리 |
| `health_check()` | public | GET /system_stats (5초 타임아웃) |
| `interrupt_current()` | public | POST /interrupt — 현재 워크플로우 중단 |
| `clear_queue()` | public | POST /queue {clear:true} — 큐 삭제 |
| `generate_t2v(prompt, ...)` | public | T2V 생성 (full/distilled 워크플로우 선택) |
| `generate_t2v_with_upscale(prompt, ...)` | public | 2-Stage 업스케일 T2V (테스트용) |
| `generate_i2v(prompt, init_image_path, ...)` | public | I2V 생성 (이미지 리사이즈+업로드 포함) |
| `_resize_image(image_path, width, height)` | private | 종횡비 가드(3:1 center-crop) + LANCZOS 리사이즈 |
| `_load_workflow(filename)` | private | workflows/ 디렉터리에서 JSON 로드 |
| `_patch_workflow(workflow, params)` | private | 키 매칭 + 값 매칭(화이트리스트) 파라미터 교체 |
| `_upload_image(image_path)` | private | POST /upload/image (multipart) |
| `_poll_until_done(prompt_id, interval=3.0)` | private | GET /history polling (3초 간격, 10회 에러 한계) |
| `_queue_and_wait(workflow, timeout)` | private | 큐 제출 + polling + 결과 추출 |
| `_wait_for_file(path, timeout=30.0)` | private | Docker 볼륨 동기화 대기 (크기 안정 확인) |

#### 상수

| 상수 | 값 | 용도 |
|------|-----|------|
| `_PLACEHOLDER_WHITELIST` | `frozenset({"positive_prompt", "negative_prompt", "init_image"})` | 값 매칭 허용 placeholder |

### prompt_engine.py (~285행)
...
| 심볼 | 역할 |
|------|------|
| `_T2V_PROMPT_SYSTEM_V2` | T2V 시스템 프롬프트 (한국 중심, 6요소, 인물 묘사 티어) |
| `_I2V_SYSTEM` | I2V 시스템 프롬프트 (동작/카메라/사운드만) |
| `NEGATIVE_PROMPT` | 네거티브 프롬프트 상수 (모든 T2V/I2V에 사용) |
| `_VIDEO_STYLES` | 모듈 레벨 캐시 변수 (video_styles.json) |
| `_load_video_styles()` | config/video_styles.json 캐시 로더 |
| `_get_style_hint(mood)` | mood → atmosphere 필드 추출 |
| `_clean_prompt(raw)` | LLM 출력 정리 (단일 단락, 따옴표/마크다운 제거) |

#### VideoPromptEngine 클래스

| 메서드 | 접근 | 역할 |
|--------|------|------|
| `generate_prompt(text_lines, mood, ...)` | public | 한국어→영어 프롬프트 (T2V/I2V 분기, LLM 호출 + 로깅) |
| `simplify_prompt(original_prompt, ...)` | public | 재시도용 2-3문장 축소 (LLM 호출 + 로깅) |
| `generate_batch(scenes, mood, ...)` | public | 씬 일괄 프롬프트 생성 + simplified 버전 미리 생성 |

#### LLM 호출 사양

| 메서드 | max_tokens | temperature | call_type |
|--------|-----------|-------------|-----------|
| `generate_prompt()` T2V | 180 | 0.4 | `video_prompt_t2v` |
| `generate_prompt()` I2V | 120 | 0.3 | `video_prompt_i2v` |
| `simplify_prompt()` | 80 | 0.2 | `video_prompt_simplify` |

### image_filter.py (~176행)
...
| 심볼 | 역할 |
|------|------|
| `ImageSuitability` | 평가 결과 데이터클래스 (score, reason, category, width, height) |
| `evaluate_image(image_path)` | 5기준 가중치 평가 (해상도 0.20, 종횡비 0.20, 텍스트 0.30, 색상 0.15, 엣지 0.15) |

#### 내부 (private)

| 함수 | 역할 |
|------|------|
| `_is_text_heavy(img)` | 히스토그램 bimodal(>70%) + 엣지(<3%) → 텍스트 캡처 감지 |
| `_color_diversity(img)` | RGB 채널별 std 평균 (높을수록 다양) |
| `_edge_density(img)` | Pillow FIND_EDGES → 엣지 픽셀 비율 (>30 threshold) |
| `_classify_image(img, reasons)` | 부적합 사유 기반 카테고리 분류 (text_capture/meme/screenshot/diagram/photo) |
...
```python
import functools, logging, subprocess
from pathlib import Path
```
...
| 함수 | 역할 |
|------|------|
| `validate_frame_count(n)` | 1+8k 규칙 보정 (최소 9) |
| `validate_resolution(width, height)` | 8의 배수 보정 (최소 64px) |
| `resize_clip_to_layout(input, output, w, h, fps)` | center-crop + resize + FPS 정규화 |
| `loop_or_trim_clip(input, output, target_duration)` | 루프/트림/복사로 길이 맞춤 |
| `normalize_clip_format(input, output, w, h, fps)` | 최종 1080x1920 h264 정규화 |
| `get_video_duration(path)` | ffprobe 길이 조회 |

#### 내부 (private)

| 함수 | 역할 |
|------|------|
| `_nvenc_available()` | `@functools.cache` NVENC 런타임 프로빙 |
| `_resolve_intermediate_codec()` | NVENC/libx264 선택 → (코덱명, FFmpeg 인자 리스트) |
...
| 파일 | 용도 |
|------|------|
| `test/test_full_pipeline_e2e.py` | 전체 파이프라인 E2E 테스트 |
| `test/test_comfy_client.py` | ComfyUIClient 단위 테스트 |
| `test/e2e_video_test.py` | 비디오 생성 E2E 테스트 |
| `test/test_ltx2_video.py` | LTX-2 직접 생성 테스트 |
| `test/test_image_filter.py` | 이미지 적합성 평가 테스트 |
| `test/test_prompt_engine.py` | 프롬프트 엔진 테스트 |
| `test/test_pipeline_phases.py` | 파이프라인 Phase별 테스트 |
| `test/test_video_manager.py` | VideoManager 테스트 |
