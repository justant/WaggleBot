...
def _nvenc_available() -> bool:
    """h264_nvenc 사용 가능 여부를 런타임 프로빙으로 판정."""
    try:
        result = subprocess.run(
            ["ffmpeg", "-y", "-f", "lavfi", "-i", "nullsrc=s=256x256:d=0.1",
             "-c:v", "h264_nvenc", "-f", "null", "-"],
            capture_output=True, text=True, timeout=10,
        )
        ok = result.returncode == 0
        if not ok:
            logger.warning("[video_utils] h264_nvenc 사용 불가 — libx264 폴백")
        return ok
    except Exception:
        logger.warning("[video_utils] h264_nvenc 프로빙 실패 — libx264 폴백")
        return False
...
def _resolve_intermediate_codec() -> tuple[str, list[str]]:
    """중간 처리용 코덱 — h264_nvenc 우선, 불가 시 libx264 폴백."""
    if _nvenc_available():
        return "h264_nvenc", ["-c:v", "h264_nvenc", "-preset", "p1", "-pix_fmt", "yuv420p"]
    return "libx264", ["-c:v", "libx264", "-preset", "ultrafast", "-pix_fmt", "yuv420p"]
