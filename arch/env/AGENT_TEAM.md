...
> 해당 도메인의 전체 코드 맥락을 모르는 Agent가 건드리면 기존 코드를 망가뜨릴 수 있다.

#### 시나리오 예시

Agent A가 새 TTS 기능을 구현하는데, Agent B의 `config/layout.json`에 `tts_enabled` 플래그가 필요한 경우:

```
┌─────────┐                 ┌──────────┐                 ┌─────────┐
│ Agent A │                 │Team Lead │                 │ Agent B │
└────┬────┘                 └────┬─────┘                 └────┬────┘
     │                          │                             │
     │ ① 크로스 도메인 요청       │                             │
     │ "layout.json에             │                             │
...
"크로스 도메인 요청.
 대상: Agent B 도메인 (config/layout.json)
 내용: layout.json에 tts_enabled boolean 플래그 추가.
 이유: 새 TTS 기능에서 씬별 TTS 활성화 여부를 판별하기 위해.
 긴급도: 현재 작업 블로킹됨 — 이 변경 없이 진행 불가.
 요청 Agent B 작업: layout.json의 각 레이아웃에 "tts_enabled": true 필드 추가."
```

**Step 2: Team Lead가 조율**

```
# Team Lead의 행동:
1. 요청자(Agent A)에게: "대기해. Agent B에게 할당하겠다."
2. Agent B에게 Sub-task 할당:
   "Agent A의 요청에 따라 config/layout.json을 업데이트하라.
    각 레이아웃에 "tts_enabled": true 필드를 추가.
    완료되면 알려라."
3. Agent B 완료 확인 후 → Agent A에게: "layout.json 업데이트 완료. 작업 재개해."
```

**Step 3: 요청자가 작업 재개**

Agent A는 변경된 layout.json을 읽기 전용으로 참조하여 자기 도메인 코드를 계속 구현.

#### 크로스 도메인 요청 메시지 필수 필드
...
| 요청 작업 | 대상 Agent가 수행할 구체적 작업 |

#### 금지 사항

- ❌ Team Lead가 요청자에게 타 도메인 임시 쓰기 권한 부여
- ❌ 요청자가 "급하니까" 직접 타 도메인 파일 수정
- ❌ Team Lead가 직접 코드를 작성하여 중재 (조율만 수행)
...
```
Read .claude/prompts/team_lead.md and CLAUDE.md and arch/env/AGENT_TEAM.md.
...
- Spawn teammates following arch/env/AGENT_TEAM.md Section 3.
...
```
# CEO → Team Lead
"ai_worker를 restructure해. arch/ai_worker_restructure.md 참조."
